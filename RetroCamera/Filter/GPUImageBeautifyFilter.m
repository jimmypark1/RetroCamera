...
- (void)process2:(GPUImageFilter*)filter
{
    ...
    [combinationFilter addTarget:filter];
   
    [filter addTarget:hsbFilter];
  
    
    
    self.initialFilters = [NSArray arrayWithObjects:  bilateralFilter,cannyEdgeFilter,combinationFilter,filter,nil];
...
{
    [hsbFilter addTarget:filter];
    self.terminalFilter = filter;
    
}
- (UIImage*)takePicture
{
  //  [hsbFilter setFrameBuffer];
    UIImage *filteredPhoto = [hsbFilter imageFromCurrentFramebuffer];
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    for (GPUImageOutput<GPUImageInput> *currentFilter in self.initialFilters)
    {
        ...
        {
            GPUImageFilter *filter = (GPUImageFilter*)currentFilter;
